...
- **Меню-бар**: показывает цветной кружок CO₂ в строке меню (🟢 норма, 🟡 повышение, 🔴 опасно). Клик по иконке показывает текущее показание (`CO₂: NNN ppm`), температуру (`Temp: NN.N°C`, если датчик её отдаёт) и время замера; выход — через Quit. Отправляет нативные уведомления macOS. Процесс отвязан от терминала — окно можно закрывать.
...
- Тренд: рост ≥ `trend_alert_ppm_per_min` ppm/мин за последние `trend_window_seconds` секунд → «CO₂ rising fast», не чаще `trend_cooldown_seconds`. Быстрое падение не уведомляет (уже покрыто возвратом в зелёную зону) и не расходует задержку тренда.
